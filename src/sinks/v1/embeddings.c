...
#include "an-openai-api-library/sinks/v1/embeddings.h"
...
{
    openai_v1_embeddings_sink_t *s = (openai_v1_embeddings_sink_t *)iface;
    fprintf(stderr, "[openai_v1_embeddings_sink] HTTP %ld, CURL %d\n", http, res);
    s->complete_callback(s->complete_callback_arg, req, false, NULL, 0, 0);
}

/* Init / destroy ---------------------------------------------------------- */
static bool sink_init(curl_sink_interface_t *iface, long _len)
{
    (void)_len;
    openai_v1_embeddings_sink_t *s = (openai_v1_embeddings_sink_t *)iface;
...
static void sink_destroy(curl_sink_interface_t *iface)
{
    openai_v1_embeddings_sink_t *s = (openai_v1_embeddings_sink_t *)iface;
...
static void sink_complete(curl_sink_interface_t *iface,
                          curl_event_request_t *req)
{
    openai_v1_embeddings_sink_t *s = (openai_v1_embeddings_sink_t *)iface;
    ...
    ajson_t *json = ajson_parse_string(
        pool, aml_buffer_data(s->response_buffer));
    aml_buffer_clear(s->response_buffer);

    if (!json || ajson_is_error(json)) {
        fprintf(stderr, "[openai_v1_embeddings_sink] JSON parse error\n");
        s->complete_callback(s->complete_callback_arg, req, false, NULL, 0, 0);
        return;
    }
    ...
    if (!data || !ajson_is_array(data)) {
        fprintf(stderr, "[openai_v1_embeddings_sink] No embeddings array\n");
        s->complete_callback(s->complete_callback_arg, req, false, NULL, 0, 0);
        return;
    }

    size_t n_emb = ajsona_count(data);
    float **vecs = aml_pool_alloc(pool, n_emb * sizeof(float *));
    if (!vecs) {
        s->complete_callback(s->complete_callback_arg, req, false, NULL, 0, 0);
        return;
    }
         ...
         el = ajsona_next(el), ++idx)
    {
        ...
        vecs[idx] = ajson_extract_float_array(
            &dim, pool, ajsono_scan(el->value, "embedding"));

        if (s->expected_embedding_size && dim != s->expected_embedding_size) {
            fprintf(stderr,
                    "[openai_v1_embeddings_sink] Unexpected dim %zu vs exp %zu\n",
...
curl_sink_interface_t *openai_v1_embeddings_sink(
    ...
    void *cb_arg)
{
    openai_v1_embeddings_sink_t *s = aml_pool_zalloc(req->pool, sizeof(*s));
    if (!s) return NULL;
